...
export function resolveLlm(env: GameEnv) {
  ...
  if (genericKey) {
    return {
      ...
      baseUrl: (
        env.LLM_BASE_URL?.trim() || "https://api.deepseek.com/v1"
      ).replace(/\/+$/, ""),
      apiKey: genericKey,
...
const GENRE_STYLE: Record<Genre, string> = {
  ...
  supernatural:
    "怪力乱神：可以有鬼神与因果，但要克制、有规矩，读起来像一则民间怪谈而非血浆片。",
};
...
async function generateWithLlm(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `${cfg.label} 返回 ${response.status}: ${detail.slice(0, 300)}`,
      );
    }
    ...
    if (!parsed.success) {
      throw new Error(
        `模型输出不符合结构: ${parsed.error.message.slice(0, 200)}`,
      );
    }
    return parsed.data;
  } finally {
...
export function pickBuiltin(
  ...
  if (theme.trim()) {
    const hit = BUILTIN_PUZZLES.find(
      (puzzle) =>
        puzzle.tags?.some((tag) => theme.includes(tag)) ||
        theme.includes(puzzle.title),
    );
    if (hit) return hit;
  }
  const pool = BUILTIN_PUZZLES.filter(
...
const HOST_QUESTIONS = {
  intent: choice(
    {
      ...
      focus:
        "Classify the intent only. Do not judge whether the player is right.",
    },
    {
      yes_no_question: {
        ...
        not_for:
          "A full or partial explanation of what happened; that is a guess.",
        examples: [
          "Was the man blind?",
      ...
      guess: {
        ...
        examples: [
          "He killed her because she cheated, right?",
          "I think the mirror was fake.",
        ],
      },
      meta: {
  ...
  verdict: choice(
    ...
    },
    {
      ...
      partly:
        "The truth is partly right and partly wrong, so a plain yes or no would mislead.",
      irrelevant:
  ...
  guess_closeness: score(
    {
      question:
        "How close is the player explanation to the hidden truth in `puzzle.truth`?",
      compare: ["latest_player_message", "puzzle.truth"],
      focus:
    ...
    [
      ...
      },
      {
        ...
        examples: [
          "He faked his death to escape debt, but who helped him is missing.",
        ],
      },
      {
  ...
  solved: noul(
    {
      ...
      compare: ["latest_player_message", "puzzle.truth"],
      focus:
        "The core plot and the key twist must both be correct. Different wording is fine.",
    },
    {
      true: {
  ...
  meta_request: choice(
    {
      question:
        "If the player is talking about the game itself, what do they want?",
      inspect: "`latest_player_message`",
...
function readLuck(value: unknown): LuckInfo | null {
  ...
  if (typeof luck.score !== "number" || typeof luck.tier !== "string")
    return null;
  return {
...
export interface PuzzleStore {
  create(
    puzzle: Puzzle,
    meta: { difficulty: string; createdAt: number },
  ): Promise<string>;
...
export async function askHost(
  ...
  if (!puzzle)
    throw new ApiError(404, "这一局已经过期了，请重新生成一碗海龟汤");
...
export async function judge(
  ...
  if (message.length > MAX_MESSAGE_CHARS)
    throw new ApiError(400, "内容太长了，缩短一点再问吧");
