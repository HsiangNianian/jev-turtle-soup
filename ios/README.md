...
广场按最新、精选、热门浏览汤友的作品。顶栏搜索进入独立页面，可边输入边看结果；最近搜索只保存在本机，可随时清空。
作者主页、点赞和留言也在原生界面中。讨论默认折叠并提示可能涉及汤底；
...
「我的」包含自己的作者主页、作品和案卷；作品列表显示各汤被主动揭晓的独立玩家数，自动补取汤底与推理通关不计入。
账号切换会清理互动缓存和私有页面，旧请求不能恢复上一账号的权限。
...
UI 测试包含三个流程并留存截图：线上只读验证广场、作者主页、讨论入口、登录、每日和续案；
隔离测试验证草稿恢复、发布、作者作品数据、点赞、留言、删除和动态；另一个流程验证搜索与历史。测试不会向线上写入测试作品或互动。
