...
type GeneratedPuzzle = z.infer<typeof PuzzleSchema>

const BUILTIN_PUZZLES: GeneratedPuzzle[] = [
  {
    title: '海边的小屋',
    difficulty: '中等',
    surface:
...
function pickBuiltin(difficulty: string, theme: string): GeneratedPuzzle {
  if (theme.trim()) {
    const hit = BUILTIN_PUZZLES.find((p) => p.tags?.some((t) => theme.includes(t)) || theme.includes(p.title))
    if (hit) return hit
  }
  const pool = BUILTIN_PUZZLES.filter((p) => !p.difficulty || p.difficulty === difficulty)
...
const HOST_QUESTIONS = {
  ...
  verdict: choice(
    {
      ...
      focus: 'Judge strictly against the hidden truth. If the message is not a yes/no question about the story, choose cannot_answer.',
    },
    {
      ...
      irrelevant: 'The question asks about a detail the truth never addresses and that does not affect the story; it is neither true nor false.',
  ...
  guess_closeness: score(
    {
      ...
      compare: ['latest_player_message', 'puzzle.truth'],
      focus: 'Only rate an explanation attempt. If the player did not propose an explanation, choose the lowest level.',
    },
    [
      {
  ...
  solved: noul(
    {
      question: 'Does `latest_player_message` fully and correctly state the core truth in `puzzle.truth`?',
      compare: ['latest_player_message', 'puzzle.truth'],
...
function handleMeta(kind: string, session: Session) {
  if (kind === 'hint') {
    return {
      intent: 'meta',
      verdict: 'hint',
      solved: false,
      revealed: false,
      reply: `主持人压低声音说了一句提示：「${session.hint}」`,
    }
  }
  if (kind === 'full_answer') {
    return {
...
function composeTurn(session: Session, answers: HostAnswers) {
  ...
  if (intent === 'guess') {
    ...
    return { intent, verdict: 'partial', solved: false, revealed: false, closeness, confidence: closeness, reply }
  }

  if (intent === 'meta') {
    return { ...handleMeta(answers.meta_request.choice as string, session), closeness: null, confidence: answers.meta_request.confidence }
  }
  ...
  if (intent === 'yes_no_question') {
    ...
    if (verdict === 'cannot_answer' || confidence < 0.35 || intentConfidence < 0.4) {
      return { intent, verdict, solved: false, revealed: false, closeness: null, confidence, reply: REPHRASE }
    }
...
function requireSession(id: unknown): Session {
  if (typeof id !== 'string' || !id) throw new HttpError(400, '缺少 sessionId')
  const session = sessions.get(id)
  if (!session) throw new HttpError(404, '这一局已经过期了，请重新生成一碗海龟汤')
  session.lastSeen = Date.now()
  return session
}

async function handleNew(env: Env, body: Record<string, unknown>) {
  pruneSessions()
  const difficulty = typeof body.difficulty === 'string' && body.difficulty.trim() ? body.difficulty.trim() : '中等'
  ...
  if (llm) {
    try {
      ...
      const fallback = pickBuiltin(difficulty, theme)
      puzzle = fallback
      source = 'builtin'
      console.warn('[turtle-soup] 生成失败，回退到内置汤：', error instanceof Error ? error.message : error)
    }
  } else {
...
async function handleAsk(env: Env, body: Record<string, unknown>) {
  const session = requireSession(body.sessionId)
  const message = typeof body.message === 'string' ? body.message.trim() : ''
  if (!message) throw new HttpError(400, '请输入内容')
  if (message.length > MAX_MESSAGE_CHARS) throw new HttpError(400, '内容太长了，缩短一点再问吧')
  ...
  const turn = composeTurn(session, answers)
  return {
    ...turn,
    truth: turn.revealed ? session.truth : undefined,
    model,
    debug: buildDebug(answers),
  }
}

async function handleReveal(body: Record<string, unknown>) {
  const session = requireSession(body.sessionId)
  return { title: session.title, truth: session.truth, hint: session.hint }
}
...
async function route(env: Env, req: IncomingMessage, res: ServerResponse) {
  const url = new URL(req.url ?? '/', 'http://localhost')
  const path = url.pathname

  if (req.method === 'GET' && path === '/api/health') {
    sendJson(res, 200, await handleHealth(env))
    return
  }
  if (req.method !== 'POST') {
  ...
  if (path === '/api/game/new') {
    sendJson(res, 200, await handleNew(env, body))
    return
  }
  if (path === '/api/game/ask') {
    sendJson(res, 200, await handleAsk(env, body))
    return
  }
  if (path === '/api/game/reveal') {
    sendJson(res, 200, await handleReveal(body))
    return
  }
  sendJson(res, 404, { error: '未知接口' })
}

async function handler(env: Env, req: IncomingMessage, res: ServerResponse) {
...
export function turtleSoupApi(env: Env): Plugin {
  return {
