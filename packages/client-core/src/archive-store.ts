...
export class ArchiveStore {
  private operation = (id: string, kind: PendingSave['kind'], imported = false): PendingSave => ({
    id,
    kind,
    imported,
    revision: this.uuid(),
  ...
  private uuid: () => string
  constructor(storage: StoragePort, uuid: () => string = () => crypto.randomUUID()) {
    this.uuid = uuid
    ...
    try {
      ...
      this.data = {
        ...
        spaces: {
          [spaceKey(owner)]: {
            games,
            pending: games.map((game) => this.operation(game.id, 'put', true)),
          },
        },
      }
      this.unreadable = false
  ...
  claimGuest(uid: string): boolean {
    ...
    for (const game of guest.games) {
      ...
      if (!existing || game.updatedAt > existing.updatedAt) {
        ...
        pending.set(game.id, this.operation(game.id, 'put', true))
      }
    }
